function [divergence,lds]=lds_calc_mehdizadeh(state,ws,fs,period,n_neighbours, plotje)
...
%           plotje: show a graph.
%           n_neighbours: the number of neighbours you want to use. 
...
%% version 1.1 (or so) with support for multiple nearest neighbours, as in Mehdizadeh  (2019) DOI: 10.1016/j.jbiomech.2019.01.013
...
for i_t = 1:m % loop over time samples
    ...
    [~,index]           = sort(sum(difference,2));
    for i_nn = 1:n_neighbours
        divergence((i_t-1)*n_neighbours+i_nn,:)   = sqrt(sum((state(i_t:i_t+ws-1,:)-state(index(i_nn):index(i_nn)+ws-1,:)).^2,2)');% track divergence, and store
    end
end
